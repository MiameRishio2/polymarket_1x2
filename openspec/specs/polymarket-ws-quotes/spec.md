...
Define provider-local Polymarket football event discovery, quote and score collection, and
immutable access to the latest normalized quote state.
...
The system SHALL expose an immutable latest quote snapshot for a known Polymarket asset so
provider-local dry-run logic can consume the same state populated by initial CLOB order books and
market WebSocket updates.
